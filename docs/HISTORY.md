...
- ``hiyontof.conf.js`` に含まれるデフォルトのサーバ一覧を更新

## 2017年07月29日

- 画像のアップロード機能を追加
	- どどんとふ Ver.1.48.28 以降でのみ有効
	- 能力表 (イニシアティブ表) から可能
	- ひよんとふの設置されているサーバと同じサーバにあるどどんとふに接続した際のみキャラ画像をアップロードした画像に即時変更

## 2017年03月11日

- チャットタブの見た目を20170129までと同様に戻す
	- 環境によっては大きくユーザビリティを損なうことが判明したため
- チャットタブにシンプル入力モードを追加
	- 文字入力欄と送信ボタンのみ
	- 細かい設定は発言欄で行う
- チャットログ読み込み時のバグを修正
- チャットパレットに関するバグを修正
- 今後はどどんとふの重要な更新への追随およびバグ修正のみを行う予定

## 2017年03月04日

- サーバにある画像を選択する機能を追加
	- ひよんとふの設置されているサーバと同じサーバにあるどどんとふに接続した際のみ有効
- チャットタブの見た目を大幅に変更
- 入室時の名前設定に関するバグを修正
- ガイドページにいくらかの情報を追加
	- 動画によるひよんとふガイドを追加
...
- 入室時に名前を設定できるように修正
	- hiyontof.html からは追加メニューから設定
	- roomlist.html からは入室時に尋ねられる
- 発言する時の色に関するバグを修正
- ガイドページへのリンクを追加
